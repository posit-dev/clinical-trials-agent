...
governed population and analysis definitions as `{{name}}` tokens, and say
explicitly that the answer came from an ad-hoc query, not a validated table.

- Scope any ad-hoc adverse-event query to analysis records with
  `{{analysis_records}}`.
- State the analysis population you used, and apply it as a token:
